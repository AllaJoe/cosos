      ...
      {showModal && (
        ...
        <ul style={{display:"flex", flexDirection:"column", gap:"50px", textDecoration:"none", listStyle:"none"}} className="">
          ...
          {secciones.map((seccion, index) => (
            <li
              key={index}
              style={{display:"flex", flexDirection:"column", justifyContent:"center", alignItems:"center", backgroundColor:"", textDecoration:"none", fontFamily:"Montserrat-light", textTransform: "uppercase", fontSize:"1.8em", fontWeight:"normal",color:"white",}}
            >
            <Link to={`/${seccion}`} style={{textDecoration:"none", color:"white"}}>
              {seccion}
            </Link>  
            </li>
          ))}
